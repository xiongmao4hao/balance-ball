...

{
  ...
  {  
    dmpdatas[dmpnum] = mpu9265_ReadReg(0x74);      
  }  
  mpu9265_WriteReg(DETECT_CTRL , 0xcc);//FIFO_RST | DMP_RST | DMP_EN | FIFO_EN
  ...
  for (datanum=0;datanum<1929;datanum++)
   {
     ...
     if(lastbank!=bank)
     { 
       mpu9265_WriteReg(0x6d,bank);
       mpu9265_WriteReg(0x6e,0); 
     }

     mpu9265_WriteReg(0x6f,dmpmemorydata[datanum]);     
     lastbank=bank; 
    };
}
...
void loadcfgupd(void)
{
  ...
  for (line=0;line<37;line++)
  {
    ...
    mpu9265_WriteReg(0x6d,bank);  
    mpu9265_WriteReg(0x6e,offset);
    for (writingcounts=0;writingcounts<bytes2write;writingcounts++)
	{                 
    	 datacounts++;
         mpu9265_WriteReg(0x6f,dmpcfgupddata[datacounts]); 
        }
       datacounts++;
  }
}
...
{
   	mpu9265_WriteReg(PWR_MGMT_1, 0x00);   // 使能温度传感器并用内部8M晶振
        mpu9265_WriteReg(SMPLRT_DIV, 0x04);   //
        mpu9265_WriteReg(CONFIG, 0x06);   //
        mpu9265_WriteReg(GYRO_CONFIG, 0x18);   //陀螺仪配置：2000度/秒满量程
        mpu9265_WriteReg(ACCEL_CONFIG, 0x00);   //加速度计配置：2g满量程
        loadfirmware(); 
        loadcfgupd();
        mpu9265_WriteReg(DMP_CFG_1 , 0x03);
        mpu9265_WriteReg(DMP_CFG_2 , 0x00);
        mpu9265_WriteReg(DETECT_CTRL , 0xcc);
}
